...
function get_sanitized_arrayJSList()
{
    global $arrayJS_list;
    ...
    if (isset($dc_subjects) && is_array($dc_subjects))
    {
        ...
        for ($i=0;$i<count($dc_subjects);$i++)
        {
            if ($i!=0)
                $arrayJS_list .= ', ';
            $arrayJS_list .= '"'.esc_js($dc_subjects[$i]).'"';
        }
        $arrayJS_list .= ');';
    }
    else
    ...
    if (isset($dc_locations) && is_array($dc_locations))
    {
        ...
        for ($i=0;$i<count($dc_locations);$i++)
        {
            if ($i!=0)
                $arrayJS_list .= ', ';
            $arrayJS_list .= '"'.esc_js($dc_locations[$i]).'"';
        }
        $arrayJS_list .= ');';
    }
    else
...
function print_sanitized_arrayJSList() 
{
   echo get_sanitized_arrayJSList(); 
}
